...
Una vez que se ha desarrollado la cinemática inversa del robot, se procedió a la implementación de diversas rutinas por medio de la Interfaz Greafica de Matlab, cada una de las cuales desempeña funciones específicas en la operación del sistema. La primera de estas rutinas, denominada "Recoger Herramienta", se encarga hábilmente de aproximarse al portamarcador y, mediante la acción precisa de la pinza, sujetar el marcador. En segundo lugar, la opción "Dibujar Espacio de Trabajo" realiza la representación gráfica de los límites máximo y mínimo del espacio en el cual el robot puede llevar a cabo sus operaciones. El siguiente botón, titulado "Escribir Iniciales", desencadena una rutina que no solo es capaz de escribir las iniciales de los integrantes del grupo, sino que también logra trazar dos figuras geométricas distintas a cada lado. Por otro lado, el botón "Dejar Herramienta" opera como gestor de la rutina destinada a devolver el marcador al portamarcador de manera controlada. Finalmente, se ha implementado la función de "Parada de Emergencia", la cual puede interrumpir cualquiera de las rutinas en curso, asegurando así una mejora sustancial en la seguridad del proceso. Es importante destacar que la interfaz gráfica presenta una serie de etiquetas (labels) que informan sobre el estado actual del robot durante la ejecución de cada rutina, brindando una retroalimentación clara y precisa sobre su desempeño.
